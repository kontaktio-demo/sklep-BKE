    ...
    <QuickViewProvider>
      <div className="mx-auto max-w-[1600px] px-4 py-16 md:px-6 md:py-24">
        ...
        <header className="border-b border-nf-border pb-10">
          <Breadcrumbs
            items={[{ label: t("breadcrumb.home"), href: "/" }, { label: t("search.title") }]}
          />
          <h1 className="type-h1 mt-6 text-nf-white">{t("search.title")}</h1>
          ...
          {query !== "" && (
            <p aria-live="polite" className="mt-6 text-sm text-nf-muted">
              {total > 0
                ? t("search.resultsSummary", { total, query })
                : t("search.noResultsSummary", { query })}
            </p>
          )}
        </header>
        ...
        {query !== "" && total === 0 && (
          <div className="py-20 text-center">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <Button href="/collections/collars">{t("search.goToShop")}</Button>
        ...
        {shopHits.length > 0 && (
          <section aria-labelledby="wyniki-sklep" className="pt-12">
            <SectionHeading
              id="wyniki-sklep"
              title={t("search.shopSection")}
              count={shopHits.length}
              href="/collections/collars"
              linkLabel={t("search.allCollars")}
            />
            <div className="pt-8">
