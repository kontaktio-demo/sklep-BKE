...
export function ProProductCard({
  ...
  return (
    <article
      className={cn(
        "group/card flex h-full flex-col bg-pro-bg p-5 transition-colors duration-250 ease-nf hover:bg-pro-surface motion-reduce:transition-none",
        className
      )}
    >
      {/* naglowek techniczny kafla: oznaczenie po lewej, klasyfikacja po prawej.
          Jedna klasa dla kazdego odczytu w sekcji Pro (type-pro-spec, Geist Mono 12 px / 0.08em) -
          wczesniej ten sam napis mial trzy rozne trackingi i kody nie stały w jednej linii */}
      <div className="type-pro-spec flex items-baseline gap-4 text-pro-muted">
      ...
      <div className="relative mt-4 aspect-[4/3] overflow-hidden border border-pro-border bg-pro-raised transition-colors duration-250 ease-nf group-hover/card:border-pro-border-hi motion-reduce:transition-none">
      ...
      <h3 className="type-pro-h3 mt-4 text-pro-white">
        <Link
          href={href}
          className="underline-offset-[6px] transition-colors duration-250 ease-nf hover:underline hover:decoration-pro-red hover:decoration-2 motion-reduce:transition-none"
        >
          {product.name}
        </Link>
      </h3>

      <p className="type-pro-meta mt-1.5 text-pro-muted">{product.tagline}</p>

      <dl className="mt-5">
        {specs.map((spec) => (
          <div
            key={spec.label}
            className="flex items-baseline justify-between gap-4 border-t border-pro-border py-2"
          >
            <dt className="type-pro-spec shrink-0 text-pro-muted">
              {spec.label}
            </dt>
            <dd className="type-pro-meta text-right text-pro-text">{spec.value}</dd>
          </div>
        ))}
        ...
        {product.variants.length > 0 && (
          <div className="flex items-baseline justify-between gap-4 border-t border-pro-border py-2">
            ...
            <dd className="type-pro-meta text-right text-pro-text">
              {product.variants.map((variant, i) => (
                <Fragment key={variant.size}>
                  {i > 0 && (
                    <span aria-hidden="true" className="text-pro-muted">
                      ,{" "}
                    </span>
                  )}
                  <span
                    aria-hidden="true"
                    className={cn(!variant.inStock && "text-pro-muted line-through")}
                  >
      ...
      <div className="mt-auto flex items-center justify-between gap-4 border-t border-pro-border pt-5">
        <PriceTag
          price={product.price}
